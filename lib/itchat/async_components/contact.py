...
def update_chatroom(self, userName, detailedMember=False):
    ...
    chatroomList = json.loads(
        self.s.post(url, data=json.dumps(data), headers=headers).content.decode(
            "utf8", "replace"
        )
    ).get("ContactList")
    if not chatroomList:
    ...
    if detailedMember:
        ...
        def get_detailed_member_info(encryChatroomId, memberList):
            ...
            data = {
                ...
                "List": [
                    {"UserName": member["UserName"], "EncryChatRoomId": encryChatroomId}
        ...
        for chatroom in chatroomList:
            ...
            for i in range(int(len(chatroom["MemberList"]) / MAX_GET_NUMBER + 1)):
                memberList = chatroom["MemberList"][
                    i * MAX_GET_NUMBER : (i + 1) * MAX_GET_NUMBER
                ]
    ...
    r = [
        self.storageClass.search_chatrooms(userName=c["UserName"]) for c in chatroomList
    ]
...
def update_local_chatrooms(core, l):
    ...
    for chatroom in l:
        ...
        if (
            len(chatroom["MemberList"]) != len(oldChatroom["MemberList"])
            and chatroom["MemberList"]
        ):
        ...
        if oldChatroom.get("ChatRoomOwner") and oldChatroom.get("MemberList"):
            owner = utils.search_dict_list(
                oldChatroom["MemberList"], "UserName", oldChatroom["ChatRoomOwner"]
            )
            oldChatroom["OwnerUin"] = (owner or {}).get("Uin", 0)
        #  - update IsAdmin
        if "OwnerUin" in oldChatroom and oldChatroom["OwnerUin"] != 0:
            oldChatroom["IsAdmin"] = oldChatroom["OwnerUin"] == int(
                core.loginInfo["wxuin"]
            )
        else:
            oldChatroom["IsAdmin"] = None
        #  - update Self
        newSelf = utils.search_dict_list(
            oldChatroom["MemberList"], "UserName", core.storageClass.userName
        )
...
def update_local_uin(core, msg):
    ...
    if uins:
        ...
        if 0 < len(uins) == len(usernames):
                ...
                if userDicts:
                    if userDicts.get("Uin", 0) == 0:
                        ...
                        if userDicts["Uin"] != uin:
                            logger.debug(
                                "Uin changed: %s, %s" % (userDicts["Uin"], uin)
                            )
                else:
                    if "@@" in username:
            ...
            logger.debug(
                "Wrong length of uins & usernames: %s, %s" % (len(uins), len(usernames))
            )
    else:
...
def get_contact(self, update=False):
    ...
    def _get_contact(seq=0):
        url = "%s/webwxgetcontact?r=%s&seq=%s&skey=%s" % (
            self.loginInfo["url"],
            int(time.time()),
            seq,
            self.loginInfo["skey"],
        ...
        try:
            ...
            logger.info(
                f"Failed to fetch contact, that may because of the amount of your chatrooms {e}"
            )
...
def set_alias(self, userName, alias):
    oldFriendInfo = utils.search_dict_list(self.memberList, "UserName", userName)
    ...
    r = self.s.post(
        url, json.dumps(data, ensure_ascii=False).encode("utf8"), headers=headers
    )
